...
def send_campaign(request: CampaignRequest):
    ...
    for row in rows:
        ...
        with connection() as conn:
            conn.execute("""
                ...
                (team_name, leader_name, leader_email, campaign, status, sent_at)
                VALUES (?, ?, ?, ?, ?, ?)
            """, (team["team_name"], team["leader_name"], team["leader_email"], request.name, status, created_at))
    ...
    return {"campaign_id": campaign_id, "sent_count": sent, "failed_count": failed, "status": final_status}
